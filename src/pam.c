...
//    printf("param : %s\n%s\n%s\n%lu\n", param->mount_point, param->device_name, param->container_path, param->container_size);
    ...
    cd = init_device(param->container_path);
    printf("init device OK\n");

    // volume mount (device name)
    r = volume_mount(param->device_name, param->mount_point);
    printf("volume mount OK\n");

    return PAM_SUCCESS;
}

int pam_sm_close_session(pam_handle_t *pamh, int __attribute__((unused)) flags, int __attribute__((unused)) argc,
                         const char __attribute__((unused)) **argv) {
